...
export class UpdateChatBotMessagesTemplateUseCase implements UseCase <SaveMessageSend, SaveMessageReturn> {
  constructor(private postMessage: ChatBotRepositories){}

  execute(param: SaveMessageSend): Observable<SaveMessageReturn> {
    return this.postMessage.updateMessageSchedule(param);
  }
}
